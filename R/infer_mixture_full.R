...
#' desired method of estimation for the population mixture proportions (MCMC, PB, FULL)
...
#' and "FULL" runs additional MCMC sweeps while updating collection allele frequencies
...
#' @param method a choice between "MCMC", "PB", "FULL" methods for estimating mixture proportions
...
#' @param full_reps how many MCMC sweeps with baseline resampling to perform after \code{reps} sweeps
#' without baseline resampling
#' @param full_burn_in how many reps to discard in the beginning of fully Bayesian MCMC
...
#'
#' @examples
#' mcmc <- infer_mixture_full(reference = small_chinook_ref,
#'                       mixture = small_chinook_mix,
#'                       gen_start_col = 5,
#'                       method = "MCMC",
#'                       reps  = 200)
#' @export
infer_mixture_full <- function(reference,
                          mixture,
                          gen_start_col,
                          method = "MCMC",
  ...
  if (method != "MCMC" && method != "PB" && method != "FULL") stop("invalid selection of mixture proportion estimation algorithm: please choose 'PB', 'MCMC'")
  ...
  big_output_list <- lapply(mixture_colls_list, function(little_mix) {
    ...
    ## regardless of whether the method is PB, FULL or MCMC, you are going to run the MCMC once, at least ##
    ...
    if (method == "FULL") {
      ...
      message("  performing ", full_burn_in, " burn-in and ", full_reps, " more sweeps of method \"FULL\"", appendLF = FALSE)
