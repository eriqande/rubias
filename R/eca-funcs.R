...
#' Draw a sample for a mixture without replacement.
#'
#' Creates random reporting unit (rho) and collection (omega) proportions, and a
#' \code{sim_colls} vector for the separation of an artificial mixture dataset
#' from the reference
#'
#' @param RU_starts a vector delineating the reporting units in \code{RU_vec};
#' generated by \code{tcf2param_list}
...
#' simulated individuals, sampled from the collections with probabilities = omega
#' @export
draw_random_samples_no_rep <- function(RU_starts, RU_vec, size = 100, alpha_repunit = 1.5, alpha_collection = 1.5, min_remaining = 0) {
  rho <- gtools::rdirichlet(1, rep(alpha_repunit, length(RU_starts) - 1))
  omega <- numeric(length(RU_vec))
  omega <- sapply(1:(length(RU_starts)-1), function(x) {
    omega[RU_vec[(RU_starts[x] + 1):RU_starts[x+1]]] <- gtools::rdirichlet(1, rep(alpha_collection, RU_starts[x + 1] - RU_starts[x])) * rho[x]
...
assess_reference_mc <- function(reference, gen_start_col, reps = 50, mixsize = 100, seed = 5,
  ...
  #   (2) omegas's  ("true" simulated mixing proportions of collections)
  #   (3) sim_coll's  (a vector giving the origin of each simulated individual in the mixture)
  sim_colls <- lapply(1:reps, function(x) simulate_random_samples(params$RU_starts,
                                                                     params$RU_vec,
                                                                     size = mixsize,
  ...
  # and finally, extract the true numbers of individuals from each collection into a data frame
  true_sim_nums <- lapply(sim_colls, function(x) dplyr::data_frame(collection = factor(names(x$sim_coll), levels = levels(reference$collection))) %>%
    dplyr::group_by(collection) %>%
    dplyr::summarise(n = n()) %>%
      tidyr::complete(collection, fill = list(n = 0)) %>%
    dplyr::ungroup())
  # check to make sure that later sampling without replacement won't exhaust reference pool
  lapply(true_sim_nums, function(x) {
   if(any(reps_and_colls$n - x$n < min_remaining)) stop("Attempted to draw more individuals for mixture than allowed from a particular collection. \n Try again with different random seed, with a lower min_remaining parameter,
                                                        or with reduced mixture size")
  })
  ...
  estimates <- lapply(1:reps, function(x) {
    coll_vec <- sim_colls[[x]]$sim_coll
    mc_mixture <- reference %>%
      dplyr::group_by(collection) %>%
      dplyr::sample_n(true_sim_nums[[x]]$n)

    # Need to separate into mixture and reference, calculate params, then use infer_mixture
    # or a non-premade variation thereof
  }) %>%
    dplyr::bind_rows(.id = "iter") %>%
    dplyr::mutate(iter = as.integer(iter))

    true_sim_nums <- true_sim_nums %>%
      dplyr::bind_rows(.id = "iter") %>%
      dplyr::mutate(iter = as.integer(iter))
