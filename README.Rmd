...
Sometimes totally unexpected things happen.  One situation we say in the California Chinook
...
[`gsi_sim`])(https://github.com/eriqande/gsi_sim) does.  This makes it much, much, faster.
...
fish to different populations---it is only there to decide whether it looks like it reasonably
could have come from the population that it was assigned to.)
```{r plot-zs}
...
The normal density is in black and our observed z_score is in blue.  They fit reasonably
well, but it is not perfect.  But the z_score statistic is still a useful check when 
individuals are wildly aberrant.  

The z-score is intended to be a quick way to identify aberrant individuals.  If you see a 
z-score in your mixture sample that is considerably less than ones you saw in the reference,
...
with components `mixture_proportions` (which looks like the tibble we got back from
it in the previous section) and `individual_posteriors`, which has all the posteriors for
...
to different reporting units is for fish from different populations.
```{r}
chin_sims_with_indivs$individual_posteriors %>%
  group_by(iter, indiv, sim_coll, repunit) %>%
...
(thanks Brendan!) it made it clear that Eric had to provide a way to simulate multiple 
...
This is a result that we expect: no appreciable difference, because the reporting units
