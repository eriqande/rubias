...
```{r infer_mixture1}
mix_est <- infer_mixture(reference = small_chinook_ref, 
...
1/(# of collections), except for the parameters for all the Central Valley Spring Run
populations, to which we would like to assign Dirichlet parameters of 4.  That can be 
...
```{r}
prior_tibble <- small_chinook_ref %>%
  count(repunit, collection) %>%
  filter(repunit == "CentralValleysp") %>%
  select(collection) %>%
  mutate(pi_param = 4)
...
```{r}
set.seed(12)
mix_est_with_prior <- infer_mixture(reference = small_chinook_ref, 
                         mixture = small_chinook_mix, 
                         gen_start_col = 5,
...
comp_mix_ests <- list(
  ...
  mutate(coll_group = ifelse(repunit == "CentralValleysp", "CV_spring", "Not_CV_spring"))
...
Yep,  different than before.  Let's look at results in text:
```{r}
comp_mix_ests
```

We see that for the most part this change to the prior changed the distribution of
fish into the Central Valley Spring reporting unit (and out of the collections that
are in reporting units that are somewhat genetically similar to Central Valley Spring).  This 
is not suprising---it is very hard to tell apart fish from those different collections.  

## Aggregating collections into reporting units

This is a simple operation in the [tidyverse](http://tidyverse.org/).  (This example
isn't super meaningful now, since each repunit has just a single collection, but that 
is the way it goes since I had to cut down the examples in this vignette to
adhere to CRAN package checking time allotments, but it still shows what you'd 
do to aggregate collections into reporting units.)
```{r aggregating}
...
enough that we can compute it by default for every fish and every population.
...
The normal density is in black and the distribution of our observed z_scores is in blue.  They 
fit reasonably
well, suggesting that there is not too much weird stuff going on overall.  (That is good!) But 
there is a bit of a thick tail on the left which may indicate individuals from colledction
that are not perfectly represented in the baseline.
...
Let's say that we want to simulate data that in particular proportions.  The
package has those saved in the data object `repunit_sim_ppns_for_vignette`.  
We will assign that to the variable `top6`, and use it:
```{r top6list}
top6 <- repunit_sim_ppns_for_vignette
top6
```
