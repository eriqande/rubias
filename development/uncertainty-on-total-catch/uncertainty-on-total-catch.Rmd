...
In order to handle uncertainty in the true total catch, in 'riubias' the assumption
is made that the true size of the total catch can be
...
one can imagine a case in which transgenerational genetic mark-recapture
...
library(tidyverse)
library(rubias)
rubi_method <-  "MCMC"  # set this to BR if you want to run everything using Ben's
                       # "Bayesian Resampling" approach in which the allele frequencies
                       # are updated according to the allocations of the fish in the
                       # mixture.
set.seed(3)
...
described above has also been implemented in this "BR" method.  We don't show them
