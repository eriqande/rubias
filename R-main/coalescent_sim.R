...
coal_rho_data$method <- factor(coal_rho_data$method, levels = levels(coal_rho_data$method)[2:1])


g <- ggplot(coal_rho_data, aes(x = true_rho, y = Estimate, colour = repunit)) +
  geom_point() +
...
d <- ggplot(data = coal_rho_dev, aes(x = method, y = MSE, fill = repunit)) +
...
prop.bias <- ggplot(data = coal_rho_dev, aes(x = method, y = mean_prop_bias, fill = repunit)) +
  geom_bar(stat = "identity") +
  facet_wrap(~repunit) +
  scale_fill_brewer(palette = "Set1") +
  guides(fill = F)
...
m.bias <- ggplot(data = coal_rho_dev, aes(x = method, y = mean_bias, fill = repunit)) +
  geom_bar(stat = "identity") +
  guides(fill = F) +
  scale_fill_brewer(palette = "Set1") +
  facet_wrap(~repunit)
